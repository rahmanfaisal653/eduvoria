<div id="edit-post-modal" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden z-50 flex items-center justify-center">
    ...
    <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg mx-4 transform transition-all duration-300">
        ...
        <div class="text-center mb-5">
            ...
            <p class="text-xs text-gray-500 mt-1">Perbarui konten, penulis, atau status postingan.</p>
        </div>
        
        {{-- FORM UPDATE --}}
        {{-- Action akan diisi otomatis oleh JavaScript: /admin/content/update/{id} --}}
        <form id="edit-post-form" method="POST" action="#">
            @csrf
            {{-- Kita pakai POST sesuai request kamu (tanpa @method PUT) --}}
            ...
            <div class="space-y-4 mb-4 max-h-96 overflow-y-auto px-1">
                ...
                {{-- 2. INPUT PENULIS --}}
                <div>
                    <label for="edit-author" class="block text-sm font-semibold text-gray-800 mb-1">Nama Penulis</label>
            ...
            <div class="flex justify-end space-x-3 pt-3 border-t mt-4">
                {{-- Tombol Batal (Menutup Modal via JS) --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Elemen Input di dalam Modal
        const inputContent = document.getElementById('edit-content');
        const inputAuthor = document.getElementById('edit-author');
        const inputStatus = document.getElementById('edit-status');

        // 2. Loop setiap tombol Edit
        editButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                e.preventDefault(); // Mencegah layar scroll ke atas

                // A. AMBIL DATA DARI TOMBOL
                ...
                if(inputAuthor) inputAuthor.value = author;
                if(inputStatus) inputStatus.value = status;

                // C. UBAH URL ACTION FORM
                // Supaya saat disimpan, dia mengarah ke ID yang benar
        ...
        // 3. LOGIKA TUTUP MODAL (Opsional, jika tombol batal belum jalan)
