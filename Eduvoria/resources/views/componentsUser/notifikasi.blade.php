        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($communities as $community)
                <div class="bg-white rounded-2xl shadow-sm hover:shadow-md transition p-4 flex flex-col">
                    <img src="{{ $community['image'] }}" class="rounded-xl mb-4 w-full h-40 object-cover" alt="{{ $community['name'] }}">
                    ...
                    <button class="mt-4 bg-emerald-600 hover:bg-emerald-700 text-white py-2 rounded-lg text-sm font-medium transition">Gabung Komunitas</button>
                </div>
            @endforeach
        </div>
    </div>
